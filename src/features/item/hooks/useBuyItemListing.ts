...
import { buyItemListing } from "../requests";
import { ItemListingResponse } from "../dataTypes";
import { useRouter } from "next/navigation";
import { PAGES } from "@/utils/pages";

export const useBuyItemListing = () => {
  ...
  const doBuyItemListing = async (itemListingId: string) => {
    if (!sessionId) return;

    setIsLoading(true);

    const response = await buyItemListing({ sessionId, itemListingId });
  ...
  return { doBuyItemListing, isLoading };
};
